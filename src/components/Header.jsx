  ...
  return (
    <>
      ...
      >
        ...
        <ul className="lg:flex justify-center items-center gap-10 hidden">
          {
            navItems.map(({link, path}) => (
              <Link key={path} className="text-black text-sm
              uppercase font-semibold cursor-pointer px-4 py-2
              rounded-lg hover:bg-themepurple
               hover:text-white"
               to={path} spy={true} smooth={true} offset={-100} duration={500}>
                {link}
              </Link>
            ))
          }
        ...
        <div id="header-icons" className="lg:flex hidden
        justify-center items-center gap-6 text-black">
            <FaSearch className="w-[20px] h-[20px] transform
             hover:scale-125 transition-transform
             duration-300 cursor-pointer hover:text-themepurple" />

             <IoPerson className="w-[20px] h-[20px] transform
             hover:scale-125 transition-transform
             duration-300 cursor-pointer hover:text-themepurple" />

             <FaHeart className="w-[20px] h-[20px] transform
             hover:scale-125 transition-transform text-red-600 
             duration-300 cursor-pointer hover:text-themepurple" />

             <div className="relative">
                <FaShoppingCart className="w-[20px] h-[20px] transform
                hover:scale-125 transition-transform text-green-600
                duration-300 cursor-pointer hover:text-themepurple" />
                <div className="bg-red-700 hover:bg-themeyellow
                text-white hover:text-black rounded-full absolute
                -top-3 -right-2 text-[14px] pl-0.5 pr-0.5 font-bold">2</div>
             </div>
        </div>
      </nav>
    </>
